...
This setting defines the provider-agnostic filter DSL for the issues in the section's table.

Don't specify `type = "issue"` for this setting. The dashboard always adds that filter for
issue sections.

You can define any combination of filters. To make it easier to read and maintain your
filters, we recommend using the `>-` syntax after the `filters` key and writing one
filter per line.
...
For more information about writing filters, see [Searching].
