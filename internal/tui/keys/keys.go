...
type KeyMap struct {
	...
	ToggleGroupByProvider key.Binding
...
var Keys = &KeyMap{
	...
	ToggleGroupByProvider: key.NewBinding(
		key.WithKeys("B"),
		key.WithHelp("B", "group by provider"),
	),
...
func rebindUniversal(universal []config.Keybinding) error {
	...
	for _, kb := range universal {
		...
		case "toggleGroupByProvider":
			key = &Keys.ToggleGroupByProvider
		case "openGithub":
			key = &Keys.OpenGithub
		case "refresh":
			key = &Keys.Refresh
